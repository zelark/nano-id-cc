...
test('formatResult default matches the known value', () => {
  const state: State = { unit: 'hour', speed: 1000, length: 21, alphabet: ALPHABET, preset: null };
...
test('formatResult treats hour and second speeds equivalently', () => {
  const perHour: State = { unit: 'hour', speed: 3600, length: 21, alphabet: ALPHABET, preset: null };
  const perSecond: State = { unit: 'second', speed: 1, length: 21, alphabet: ALPHABET, preset: null };
  assert.equal(formatResult(perHour), formatResult(perSecond));
});

test('formatResult deduplicates the alphabet', () => {
  const withDuplicates: State = { unit: 'hour', speed: 1000, length: 21, alphabet: 'aabbcc', preset: null };
  const deduplicated: State = { unit: 'hour', speed: 1000, length: 21, alphabet: 'abc', preset: null };
